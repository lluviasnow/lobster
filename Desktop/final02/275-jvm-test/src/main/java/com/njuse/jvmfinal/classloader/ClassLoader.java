...
public class ClassLoader {
    ...
    private JClass loadNonArrayClass(String className, EntryType initiatingEntry) throws ClassNotFoundException {
        try {
            if(className.contains("."))className.replace(".", File.separator);
    ...
    private JClass  defineClass(byte[] data, EntryType definingEntry) throws ClassNotFoundException {
         ...
         * add to method area
         */
        clazz.setEntry(definingEntry);
        resolveSuperClass(clazz);
        resolveInterfaces(clazz);
        String thisName=clazz.getName();
        methodArea.addClass(thisName,clazz);
        return clazz;
    }
    ...
    private void resolveInterfaces(JClass clazz) throws ClassNotFoundException {
         ...
         */
        String[] interfaceList=clazz.getInterfaceNames();
        int num=interfaceList.length;
        JClass interfaces[]=new JClass[num];
        for(int i=0;i<num;i++){
            interfaces[i]=loadClass(interfaceList[i],clazz.getLoadEntryType());
        }
    ...
    private void initDefaultValue(JClass clazz, Field field) {
        ...
        if(chars[0]=='I'||chars[0]=='Z'||chars[0]=='B'||chars[0]=='C'){
            vars.setInt(field.getSlotID(),0);
        }
        if(chars[0]=='F'){
            vars.setFloat(field.getSlotID(),0);
        }
        if(chars[0]=='J'){
            vars.setLong(field.getSlotID(),0);
        }
        if(chars[0]=='D'){
            vars.setDouble(field.getSlotID(),0.0);
        }
        if(chars[0]=='L'){
            vars.setObjectRef(field.getSlotID(),new NullObject());
        }
